...
class SignupRequestTest : DescribeSpec({
    ...
    describe("SignUpRequest 유효성 검사") {
        ...
        context("올바른 데이터가 제공되었을 때") {
            it("유효성 검사를 통과해야 한다") {
                val request = SignupRequest(username = "testuser", password = "Password123")
        ...
        context("잘못된 아이디가 제공되었을 때") {
            it("아이디에 대한 유효성 검사가 실패해야 한다") {
                val request = SignupRequest(username = "invalidUsername123", password = "Password123")
                val violations: Set<ConstraintViolation<SignupRequest>> = validator.validate(request)
        ...
        context("잘못된 비밀번호가 제공되었을 때") {
            it("비밀번호에 대한 유효성 검사가 실패해야 한다") {
                val request = SignupRequest(username = "testuser", password = "short")
                val violations: Set<ConstraintViolation<SignupRequest>> = validator.validate(request)
                violations.size shouldBe 1
        ...
        context("아이디와 비밀번호 모두 잘못된 경우") {
            it("두 필드 모두 유효성 검사가 실패해야 한다") {
                val request = SignupRequest(username = "invalidUser123", password = "short")
                val violations: Set<ConstraintViolation<SignupRequest>> = validator.validate(request)
